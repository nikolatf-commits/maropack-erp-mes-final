...
export function mapDbMaster(m, nalozi = []) {
  ...
  return {
    ...
    ...m,
    id: m.id,
    broj: m.broj || podaci.broj || podaci.ponBr,
    broj_naloga: m.broj || podaci.broj_naloga || podaci.ponBr,
    ponBr: m.broj || podaci.ponBr,
...
export async function fetchCoreData() {
  const [proizvodi, ponude, nalozi, masterNalozi, rolne, masine, radnici, sessions, qc] = await Promise.all([
    ...
    supabase.from("qc_zapisnici").select("*").order("created_at", { ascending: false }),
  ]);

  const firstError = [proizvodi, ponude, nalozi, masterNalozi, rolne, masine, radnici, sessions, qc].find((r) => r.error)?.error;
  if (firstError) throw firstError;

  const mappedNalozi = (nalozi.data || []).map(mapDbNalog);
  const mappedMasters = (masterNalozi.data || []).map((m) => mapDbMaster(m, mappedNalozi));

  return {
    proizvodi: proizvodi.data || [],

...
async function kreirajNalogeOdProizvoda(p = {}, user = 'system') {
    try {
        ...
        // 2) Napravi ponudu sa res.template (da nalog povuče ceo dizajn/slojeve)
        const payload = {
            datum: new Date().toLocaleDateString('sr-RS'),
            kupac: p.kupac || prod.kupac || 'AI nalog',
            naziv, proizvod: naziv,
            tip: prod.tip || p.tip || 'folija',
            tip_proizvoda: prod.tip_proizvoda || prod.tip || p.tip || 'folija',
            kol: kolicina, kolicina,
            status: 'prihvaceno',
            proizvod_id: prod.id,
            res: { template: tpl, kupac: p.kupac || prod.kupac || '', kolicina },
            mats: (folija.layers || prod.struktura_materijala || []),

...
export default function FormatiranjePoPotrebi({ msg }) {
  ...
  async function kreiraj() {
    ...
    try {
      ...
      const redovi = rezultat.nalozi.map((n) => {
        ...
        return {
          ...
          tip_proizvoda: gVrsta(m) || null,
          parametri: JSON.stringify({ formatiranje: plan }),
          uradjeno: 0, skart: 0,
        };
      });
      // --------------------------------------------------------------------------
      ...
      if (error) throw error;
      say(`Kreirano ${redovi.length} nalog(a) za formatiranje: ${rezultat.nalozi.map((n) => n.broj).join(", ")}`);
    } catch (e) {

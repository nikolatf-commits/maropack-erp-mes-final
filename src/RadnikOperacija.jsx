...
// TODO: povući iz prave tabele radnika kad bude spremna
const RADNICI = ["Marko P.", "Jelena S.", "Nikola T.", "Operator 1"];
...
export default function RadnikOperacija({ opid }) {
  ...
  const [radnik, setRadnik] = useState(RADNICI[0]);
  ...
  async function pocni() {
    if (!radnik || !masina) { setErr("Izaberi radnika i mašinu."); return; }
    setBusy(true); setErr("");
    const { error } = await supabase.from("operativni_nalozi")
      .update({ status: "radi", radnik, masina, start_ts: op.start_ts || new Date().toISOString(), stop_ts: null, pauza_ts: null })
      .eq("id", opid);
    ...
    <div style={wrap}>
      ...
      {!showRazlog && !showFinish && (!op || op.status === "ceka" || !op.start_ts) && op?.status !== "zavrseno" && (
        <div>
          ...
          <select style={inp} value={radnik} onChange={(e) => setRadnik(e.target.value)}>{RADNICI.map((r) => <option key={r}>{r}</option>)}</select>
